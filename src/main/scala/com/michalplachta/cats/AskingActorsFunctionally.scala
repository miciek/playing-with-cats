...
object AskingActorsFunctionally extends App {
  ...
  object ClassicApproach {
    ...
    class StringStatsCalculator(lengthCalculator: ActorRef,
                                palindromeChecker: ActorRef)
        extends Actor {
      ...
      def receive = {
        case subject: String ⇒
          ...
          val futurePalindromeCheck =
            (palindromeChecker ? subject).mapTo[Boolean]
          val futureStats = for {
            length ← futureLength
    ...
    def runStringStatsApp(subject: String): StringStats = {
      ...
      implicit val timeout: Timeout = 1.second
      val lengthCalculator =
        system.actorOf(Props[LengthCalculator], "lengthCalculator")
      val palindromeChecker =
        system.actorOf(Props[PalindromeChecker], "palindromeChecker")

      val statsCalculator = system.actorOf(Props(classOf[StringStatsCalculator],
                                                 lengthCalculator,
                                                 palindromeChecker),
                                           "statsCalculator")
  ...
  object FunctionalApproach {
    ...
    def askActor[Q, A: ClassTag](actor: ActorRef): ReaderT[Future, Q, A] =
      ReaderT { (question: Q) ⇒
    ...
    def runStringStatsApp(subject: String): StringStats = {
      ...
      val calculateStats =
        stringStats(askActor(lengthCalculator), askActor(palindromeChecker))
  ...
  println(
    s"[ClassicApproach] StringStats for 'abba': ${ClassicApproach.runStringStatsApp("abba")}")
  println(
    s"[FunctionalApproach] StringStats for 'abba': ${FunctionalApproach.runStringStatsApp("abba")}")
}
